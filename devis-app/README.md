...
## 🚀 Guide de Déploiement en Production (Vercel & Supabase)

L'hébergement d'une application **Next.js** s'effectue de manière optimale et gratuite sur la plateforme **Vercel** (les créateurs de Next.js). Voici la procédure pas-à-pas à joindre ou à réaliser pour déployer le projet en production :

### Étape 1 : Mettre le projet sur GitHub
1. Créez un nouveau dépôt (public ou privé) sur votre compte [GitHub](https://github.com).
2. Dans votre terminal, à la racine du projet, initialisez Git et poussez le code :
   ```bash
   # Ajouter et valider tous les fichiers locaux
   git add .
   git commit -m "feat: complete BioControl full-stack prototype with Supabase"

   # Associer votre dépôt distant et pousser le code
   git remote add origin https://github.com/VOTRE_PSEUDO/NOM_DU_DEPOT.git
   git branch -M main
   git push -u origin main
   ```

### Étape 2 : Lancer le déploiement sur Vercel
1. Rendez-vous sur le tableau de bord de [Vercel](https://vercel.com) et connectez-vous avec votre compte GitHub.
2. Cliquez sur le bouton **`Add New`** > **`Project`**.
3. Dans la liste de vos dépôts GitHub importés, trouvez votre dépôt de devis et cliquez sur **`Import`**.

### Étape 3 : Configurer les Variables d'Environnement (Crucial)
Avant de cliquer sur le bouton de déploiement, développez la section **`Environment Variables`** de la page d'import Vercel et ajoutez les clés suivantes (avec les mêmes valeurs que votre fichier local `.env.local`) :

| Nom de la variable | Valeur de Production |
| :--- | :--- |
| `NEXT_PUBLIC_SUPABASE_URL` | *L'adresse URL réelle de votre projet Supabase* |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | *Votre clé anon publique Supabase* |
| `SUPABASE_SERVICE_ROLE_KEY` | *Votre clé secrète service_role Supabase* |
| `ADMIN_PASSWORD` | `AdminPestControl2026!` *(Ou le mot de passe admin de votre choix)* |
| `ADMIN_BEARER_TOKEN` | `secret_bearer_token_2026` *(Jeton pour les tests API automatisés)* |

### Étape 4 : Déployer !
1. Cliquez sur le bouton vert **`Deploy`**.
2. Vercel va installer les modules, compiler le TypeScript et empaqueter le projet Next.js en moins de 60 secondes.
3. **Félicitations !** Vercel vous fournit une URL publique et sécurisée (ex: `https://votre-projet.vercel.app`) accessible depuis n'importe quel smartphone, tablette ou ordinateur dans le monde.

> [!TIP]
> **Intégration continue automatique** : À partir de maintenant, chaque fois que vous ferez un changement sur votre ordinateur et que vous le pousserez sur GitHub (`git push`), Vercel détectera la mise à jour et redéploiera le site public en production automatiquement et de manière totalement invisible !
